...
def generate_event_matrix(template_miner):
    ...
    for cluster in template_miner.drain.clusters:
        # Ici, nous extrayons les données que vous souhaitez utiliser dans la matrice des événements
        # Par exemple, si chaque cluster a un attribut 'events' ou un autre attribut, on le récupère ici
        # (modifiez cela selon les attributs spécifiques de votre classe `LogCluster`)
        cluster_data = {
            'cluster_id': cluster.id,  # Supposons que chaque cluster ait un ID
            'event_count': len(cluster.events),  # Par exemple, le nombre d'événements dans le cluster
            'events': cluster.events  # Liste des événements (ou autre information pertinente)
        }
    ...
    # Enregistrer la matrice comme fichier CSV (si nécessaire)
